...
namespace WebRunLocal.Controllers
{
    ...
    ///  华天打印 controller.
    /// </summary>
    [RoutePrefix("api/nj_ht_auto")]
    public class NJHTPrintController : ApiController
    {
        [Route("print_inner")]
        [HttpPost]
        [ActionFilter]
        public IHttpActionResult PrintLabel([FromBody] NJHTItemInfo item)
        {
            ...
            if (!string.IsNullOrEmpty(item.CustomDevice))
            {
                names.Add("CustomDevice");
                values.Add(item.CustomDevice);
            }
            ...
            if (!string.IsNullOrEmpty(item.UniqueCode))
            {
                names.Add("UniqueCode");
                values.Add(item.UniqueCode);
            }
    ...
    public class NJHTItemInfo
    {
        ...
        public string CustomDevice { get; set; }
